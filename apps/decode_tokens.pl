...
#for ( 0 .. 6 ) {
#  my $token = HTTP2::Draft::Compress::get_token( \@bytes );
#  print Dumper( $token );
#  print "Remaining bytes: ", scalar @bytes, "\n";
#}

#$token = HTTP2::Draft::Compress::get_token( \@bytes );
#print Dumper( $token );
#print "Remaining bytes: ", scalar @bytes, "\n";
...
while( my $token = HTTP2::Draft::Compress::get_token( \@bytes ) ) {
  print Dumper( $token );
}
